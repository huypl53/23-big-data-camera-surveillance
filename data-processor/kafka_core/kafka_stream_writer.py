...
from utility.common_util import CLIENT_ID
...
class KafkaStreamWriter(Generic[T], StreamWriter):
    ...
    def write(self, streams: List[T]) -> None:
        ...
        for event in streams:
            ...
            self.kafka_producer.send(topic=self.topic, key=key, value=event)
    ...
    def __create_kafka_producer(self) -> KafkaProducer:
        ...
        return KafkaProducer(
            ...
            key_serializer=get_ser_des(self.config.get("key_serializer", "STRING_SER")),
            value_serializer=get_ser_des(
                self.config.get("value_serializer", "STRING_SER")
            ),
